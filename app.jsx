...
function App(){
  ...
  useEffect(()=>{
    ...
    async function initFirestore() {
      try {
        const docRef = window.firestoreDoc(window.firestoreDB, "user-data", "default");
        ...
        if (docSnap.exists()) {
          ...
          if (data.days) {
            const merged = D0.map(def => {
              const s = data.days.find(x => x.n === def.n);
              if (!s) return def;
        ...
        // Setup real-time listener for sync
        window.firestoreOnSnapshot(docRef, (doc) => {
          if (doc.exists() && ready) {
            const data = doc.data();
            // Sync from cloud if data changed externally
            console.log("☁️ Sync from cloud");
          }
        });
  ...
  const persist = useCallback(async (d) => {
    ...
    try {
      const docRef = window.firestoreDoc(window.firestoreDB, "user-data", "default");
      await window.firestoreSetDoc(docRef, { days: d }, { merge: true });
  ...
  const persistM = useCallback(async (m) => {
    ...
    try {
      const docRef = window.firestoreDoc(window.firestoreDB, "user-data", "default");
      await window.firestoreSetDoc(docRef, { memos: m }, { merge: true });
  ...
  const exportData = async () => {
    try {
      ...
      if (window.firestoreDB) {
        const docRef = window.firestoreDoc(window.firestoreDB, "user-data", "default");
        await window.firestoreSetDoc(docRef, { lastBackup: Date.now() }, { merge: true });
      }
