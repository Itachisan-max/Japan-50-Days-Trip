  ...
  useEffect(()=>{
    ...
    const unsubscribe = docRef.onSnapshot((doc) => {
      if (doc.exists) {
        ...
        // Merge with defaults
        if (data.days) {
          // Merge: use saved data, fill missing fields with defaults
          const merged = D0.map(def => {
            const saved = data.days.find(x => x.n === def.n);
            if (!saved) return def;
            // Override defaults with ALL saved fields
            return {
              ...def,           // Start with defaults
              ...saved,         // Override with everything saved
              exp: saved.exp || [],  // Ensure arrays are never null
              schedule: saved.schedule || def.schedule || [],
              a: saved.a || def.a || [],
              m: saved.m || def.m || []
            };
          });
        ...
        docRef.set({
          ...
          lastBackup: Date.now(),
          createdAt: Date.now()
        }).then(() => console.log('✅ Initial data saved'));
      }
