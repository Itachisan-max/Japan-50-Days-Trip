  ...
  return(
    <div style={{maxWidth:860,margin:"0 auto",padding:"16px 12px"}}>
      <div style={{background:"white",borderRadius:8,padding:"24px",boxShadow:"0 2px 8px rgba(0,0,0,.08)"}}>
        ...
        <p style={{margin:"0 0 20px",fontSize:14,color:"#666",fontWeight:600}}>
          {GUIDA_INFO.subtitle}
        </p>
        
        <div style={{background:"#FFF9E6",border:"2px solid #FFD700",borderRadius:6,padding:"16px",marginBottom:20}}>
          <p style={{margin:"0 0 12px",fontSize:14,color:"#333",lineHeight:1.6}}>
            <strong>📄 File completo:</strong> {GUIDA_INFO.description}
          </p>
          <div style={{background:"#333",color:"#00FF00",padding:"12px",borderRadius:4,fontFamily:"monospace",fontSize:12}}>
            📁 giappone-pwa-firebase/<br/>
            &nbsp;&nbsp;&nbsp;└── 📄 {GUIDA_INFO.fileLink}
          </div>
        </div>

        <h3 style={{margin:"24px 0 12px",fontSize:18,fontWeight:800,color:"#C1121F"}}>
          📍 Panoramica Fasi del Viaggio
        </h3>
        
        {GUIDA_INFO.sections.map((section,i)=>(
          <div key={i} style={{background:"#F8F8F8",borderLeft:"4px solid #1B4332",padding:"12px 16px",marginBottom:12,borderRadius:4}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
              <div>
                <strong style={{fontSize:15,color:"#1B4332"}}>{section.name}</strong>
                <span style={{marginLeft:8,fontSize:12,color:"#666",fontWeight:600}}>({section.range})</span>
              </div>
            </div>
        ...
        <div style={{marginTop:24,padding:16,background:"#E8F5E9",borderRadius:6,border:"1px solid #4CAF50"}}>
          <p style={{margin:0,fontSize:13,color:"#2E7D32",fontWeight:600}}>
            💡 <strong>Come usare:</strong> Ogni giorno nella vista "App Itinerario" ha il programma orario dettagliato. 
            Per approfondimenti, chicche nascoste e consigli extra, consulta il file GUIDA-DETTAGLIATA.md.
          </p>
        </div>
      </div>
    </div>
